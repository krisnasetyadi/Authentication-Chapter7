...
const initializePassport = require('./config/pass')
...
const initializepay = require('./config/pymnt_pass')
...
pool.query(
 ...
 app.put('/users/payment/:id' , async (req , res)=>{
     try {
         ...
         const updateCard = await pool.query(
             `UPDATE user_payments SET cardnumber = $1 WHERE id = $2`,
             [cardnumber,id])
 ...
 app.delete('/users/payment/:id' , async (req , res)=>{
    try {
        const {id} = req.params
        const deleteCard = await pool.query(`DELETE FROM user_payments
